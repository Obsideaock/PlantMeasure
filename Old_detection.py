...
def segment_and_count_leaves_bgr(
    bgr: np.ndarray,
    stem_suppression_radius: int = 3,
    fg_distance_thresh: float = 0.20,
...
class LeafGUI(tk.Tk):
    def __init__(self):
        ...
        self.var_radius   = tk.IntVar(self, 3)
        self.var_fg_x100  = tk.IntVar(self, 20)
        self.var_min_area = tk.IntVar(self, 500)
        ...
        self.after(50, lambda: self.pw.paneconfigure(self.ctrl_frame, minsize=360))
    ...
    def _render_only(self):
        ...
        cw = max(1, self.canvas.winfo_width())
        ch = max(1, self.canvas.winfo_height())
        h0, w0 = self.color_labels_rgb.shape[:2]
        scale = min(cw / w0, ch / h0)
        new_w = max(1, int(w0 * scale))
        new_h = max(1, int(h0 * scale))

        img = Image.fromarray(self.color_labels_rgb)
    ...
    def _compute_and_render(self):
        ...
        (self.leaf_count,
         self.leaf_mask,
         self.leaf_labels,
         self.overlay_bgr) = segment_and_count_leaves_bgr(
            self.bgr, stem_radius, fg_distance_thresh, min_leaf_area
        )
